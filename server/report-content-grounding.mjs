...
const CURRENT_PLAN_SENTENCE_PATTERN = /\b(?:durante\s+la\s+valoracion\s*,?\s*(?:(?:el|la)\s+(?:psiquiatra|profesional|facultativ[oa])\s+)?(?:se\s+)?(?:revisa(?:n)?|indica(?:n)?|pauta(?:n)?|prescribe(?:n)?|informa(?:n)?|recoge(?:n)?|detalla(?:n)?|explica(?:n)?)|(?:se|le)\s+(?:indica(?:n)?|pauta(?:n)?|prescribe(?:n)?|inicia(?:n)?)|(?:el|la)\s+(?:psiquiatra|profesional|facultativ[oa])\s+(?:indica|pauta|prescribe|inicia|explica)|pauta\s+(?:farmacologica|terapeutica)(?:\s+a\s+seguir)?|pauta\s+indicada|tratamiento\s+final)\b/i;
...
const CURRENT_MEDICATION_ASSERTION_PATTERN = /\b(?:(?:actualmente|ahora\s+mismo|en\s+la\s+actualidad)\s+(?:se\s+)?(?:toma|tomando|esta\s+tomando)|(?:la\s+)?paciente\s+(?:toma|esta\s+tomando)\s+actualmente|(?:durante\s+la\s+entrevista\s+)?(?:refiere|informa(?:\s+de)?)\s+(?:(?:que\s+)?(?:estar\s+)?tomando|tratamiento\s+actual)|tratamiento\s+habitual)\b/i;
...
function cleanClinicalMetaPhrasing(section, warnings, warningCode) {
  ...
  let text = String(original)
    ...
    .replace(/(?:una\s+)?medicaci[oó]n\s+nocturna\s+de\s+(\d+(?:[.,]\d+)?\s*(?:mg|miligramos?))\s+identificada\s+en\s+(?:el\s+discurso|la\s+transcripci[oó]n)\s+como\s+[«\"']([^»\"']+)[»\"']/gi, "«$2» $1 antes de dormir")
...
const COLLATERAL_MSE_CLAUSE_PATTERN = /^(?:segun\s+(?:la|el)\s+(?:madre|padre|familiar|acompanante|cuidador(?:a)?)|(?:(?:la|su)\s+)?(?:madre|padre|familia(?:r|res)?|acompanante|cuidador(?:a)?)\s+(?:refiere|describe|observa|senala|informa|comenta|explica|discrepa|considera)|discrepa\s+respecto\s+a(?:l)?\s+(?:sueno|descanso))\b/i;
