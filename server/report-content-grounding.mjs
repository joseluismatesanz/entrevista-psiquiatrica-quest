...
const CURRENT_MEDICATION_ASSERTION_PATTERN = /\b(?:(?:actualmente|ahora\s+mismo|en\s+la\s+actualidad)\s+(?:se\s+)?(?:toma|tomando|esta\s+tomando)|(?:durante\s+la\s+entrevista\s+)?refiere\s+(?:(?:que\s+)?(?:estar\s+)?tomando|tratamiento\s+actual)|tratamiento\s+habitual)\b/i;
