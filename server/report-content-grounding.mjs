...
const CURRENT_PLAN_SENTENCE_PATTERN = /\b(?:durante\s+la\s+valoracion\s*,?\s*(?:(?:el|la)\s+(?:psiquiatra|profesional|facultativ[oa])\s+)?(?:se\s+)?(?:revisa|indica|pauta|prescribe|informa|recoge|detalla|explica)|(?:se|le)\s+(?:indica|pauta|prescribe|inicia)|(?:el|la)\s+(?:psiquiatra|profesional|facultativ[oa])\s+(?:indica|pauta|prescribe|inicia|explica)|pauta\s+(?:farmacologica|terapeutica)(?:\s+a\s+seguir)?|pauta\s+indicada|tratamiento\s+final)\b/i;
...
const COLLATERAL_MSE_CLAUSE_PATTERN = /^(?:(?:(?:la|su)\s+)?(?:madre|padre|familia(?:r|res)?|acompanante|cuidador(?:a)?)\s+(?:refiere|describe|observa|senala|informa|comenta|explica|discrepa|considera)|discrepa\s+respecto\s+a(?:l)?\s+(?:sueno|descanso))\b/i;
